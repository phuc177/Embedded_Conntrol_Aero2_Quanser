...
typedef RT_MODEL_q_aero2_half_quad_pd_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_aero2_half_quad_pd(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_aero2_half_quad_pd_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(q_aero2_half_quad_pd_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_aero2_half_quad_pd -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (q_aero2_half_quad_pd_DW.HILReadEncoderTimebase_Task, (t_clock)
         q_aero2_half_quad_pd_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
